	...
	//	Show other profile fields
		...
		foreach ($userfields as $userfield) {	
			...
			$qa_content['form_profile']['fields'][$userfield['title']]=array(
                ...
                if(qa_user_level_string($useraccount['level'])=='Expert'){
                    require_once QA_INCLUDE_DIR.'qa-db-users.php';
                    $hehe = qa_db_users_get_expert_profile($userid);
                    if($hehe){
                        $qa_content['form_profile']['fields']['skill']=array(
                                'label'=>'Skill:',
                                'type' => 'static',
                                'tags' => 'NAME="skill" ID="skill"',
                                'value' => $hehe[0]['skill'],
                        );
                        $qa_content['form_profile']['fields']['degree']=array(
                                'label'=>'Degree:',
                                'type' => 'static',
                                'tags' => 'NAME="degree" ID="degree"',
                                'value' => $hehe[0]['degree'],
                        );
                        $qa_content['form_profile']['fields']['crop']=array(
                                'label'=> 'Crop:',
                                'type' => 'static',
                                'tags' => 'NAME="crop" ID="crop"',
                                'value' => $hehe[0]['crop']
                        );
                        $qa_content['form_profile']['fields']['Resume']=array(
                                'label'=> 'Resume:',
                                'type' => 'static',
                                'tags' => 'NAME="CV" ID="CV"',
                                'value' => "<a target='blank' href='qa-cv-upload/" . $hehe[0]['resume'] . "'> CV </a>",
                        );
                    }
                }
