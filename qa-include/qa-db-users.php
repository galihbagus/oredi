...
*/
	{
        ...
        function qa_db_user_create_expert($userid, $degree, $skill, $crop, $resume=null)
/*
	Create a new user in the database with $email, $password, $handle, privilege $level, and $ip address
*/
	{
		...
		qa_db_query_sub(
			'INSERT INTO ^userexperts '.
			'VALUES ($, $, $, $, $)',
			$userid, $degree, $skill, $crop, $resume
		);
...
*/
	{
        ...
        function qa_db_users_get_expert_profile($userid)
/*
	Get the information required for sending a mailing to the next $count users with userids greater than $lastuserid
*/
	{
		return qa_db_read_all_assoc(qa_db_query_sub(
			'SELECT * FROM ^userexperts WHERE userid=$',
			$userid
		));
	}
